...
#define def_op(which)                                                          \
  ...
  def_op(==) def_op(!=) def_op(<) def_op(>) def_op(<=) def_op(>=)
...
//----[ Stream manipulators ]---------------------------------------------------
/**
 * @brief Output colour codes from CLIAttr enums.
 */
...
/**
 * @brief Output status messages from Status enums.
 */
...
public:
  ...
  bool push_section(uint32_t line_number, std::string name) {
    ...
    if (!exiting_ && (next_section_.empty() ||
                      ((unsigned long)level_ < next_section_.size() &&
...
inline void print_help(const char *exe_name, const std::string &msg) {
  ...
  std::cout << "\t-t TAGS   Run test blocks tagged with any of the specified "
               "tags (semicolon-separated list).\n";
  std::cout << "\t-x TAGS   Run only test blocks tagged with *all* of the "
               "specified tags (semicolon-separated list).\n";
  std::cout
